CREATE OR REPLACE FUNCTION public.create_notificacao(p_user_id uuid, p_titulo text, p_mensagem text DEFAULT NULL::text, p_tipo text DEFAULT NULL::text, p_referencia_id uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- NEW: Allow if caller owns the slot (establishment owner broadcast/notification)
